...
app.post('/events', async (c) => {
  ...
  try {
    ...
    // Extract fields from request body
    const { experiment_id, event_type, event_data, session_id, visitor_id } = body
    ...
    const eventHash = await computeEventHash(experiment_id, event_type, session_id, event_data)
...
app.post('/experiments/:id/learning_memos', async (c) => {
  ...
  try {
    ...
    if (!week_number || !week_start || !week_end || !observations) {
      const errorResponse: ErrorResponse = {
        ...
        error: {
          code: 'INVALID_REQUEST',
          message:
            'Missing required fields: week_number, week_start, week_end, and observations are required',
          request_id: requestId,
        },
      }
      return c.json(errorResponse, 400)
    ...
    try {
      ...
      const parsedMemo = {
        ...createdMemo,
        metrics_snapshot: createdMemo?.metrics_snapshot
          ? JSON.parse(createdMemo.metrics_snapshot as string)
          : null,
      }
...
app.post('/payments/webhook', async (c) => {
  ...
  try {
    ...
    const sigElements = signature.split(',').reduce(
      (acc, element) => {
    ...
    if (eventType === 'payment_intent.succeeded') {
      ...
      // Log failure event
      await c.env.DB.prepare(`INSERT INTO event_log (event_type, event_data) VALUES (?, ?)`)
      ...
      await c.env.DB.prepare('UPDATE payments SET status = ? WHERE stripe_payment_intent_id = ?')
        .bind('refunded', paymentIntentId)
        .run()

      // Update lead payment_status
      await c.env.DB.prepare('UPDATE leads SET payment_status = ? WHERE stripe_payment_id = ?')
        .bind('refunded', paymentIntentId)
        .run()
      ...
      await c.env.DB.prepare(`INSERT INTO event_log (event_type, event_data) VALUES (?, ?)`)
        .bind(
          'charge_disputed',
          JSON.stringify({ payment_intent_id: paymentIntentId, dispute_id: dispute.id })
        )
        .run()
    }
...
app.post('/metrics', async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare(upsertQuery)
      .bind(
        experiment_id,
        date,
        source || null,
